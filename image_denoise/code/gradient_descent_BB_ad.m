function [denoised_image, grad_norms, func_values, psnr_values] = gradient_descent_BB_ad(gray_image, y, lambda, iters, O)
%梯度下降采用BB步长实现
%输入：噪声图像y，正则化系数lambda，最大迭代次数iters，原始图像gray_image(用于计算psnr)
    ...
    for iter = 1:iters
        %计算函数值及梯度
        [value, grad] = gradient_value_ad(x, y, O, lambda);
        ...
        if iter > 1
            ...
            step = min(max(step, 1e-3), 1);  % 限制步长范围，避免过大或过小
        end
